...
function modChip(name, value) {
  ...
  let cls = 'neutral';
  if (!isNaN(num)) {
    const negIsGood = /cost|unrest|corruption|inflation|decay|attrition(?!.*hostile)|impact|demand|liberty_desire|aggressive_expansion|build_time|interest|ae_impact|core_creation|harsh|stab_hit|power_cost|regiment_cost|ship_cost|maintenance|war_exhaustion|damage_received|morale_damage_received|landing_penalty|uprising_chance|cooldown|governing_cost|transport_cost|overextension|years_of_nationalism|envoy_travel_time/.test(name) && !/hostile_attrition|^reduced_/.test(name);
    if (negIsGood) {
