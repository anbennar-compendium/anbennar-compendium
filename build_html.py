...
function triggerToText(key, val) {
  const map = {
    ...
    'num_of_owned_provinces_with': (() => {
      ...
      if (/has_country_modifier\s*=\s*(\w+)/.test(val)) { const cm = val.match(/has_country_modifier\s*=\s*(\w+)/); conditions.push('has modifier: ' + modRef(cm[1])); }
...
function renderRequirements(trigger) {
  ...
  if (mechanical.length > 0) {
    ...
    mechanical.forEach(p => {
      ...
      const pad = indent * 16;
      if (p.type === 'logic') {
        html += `<li style="color:var(--purple);padding-left:${pad}px;list-style:none;margin-top:4px;font-style:italic;">${p.text}</li>`;
      } else {
